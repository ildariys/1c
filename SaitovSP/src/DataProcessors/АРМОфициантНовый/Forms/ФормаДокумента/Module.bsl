...
Процедура ПриОткрытии(Отказ)
	...
	//ПодключитьОбработчикОжидания("Обновить", 10);//TODO не корректно работает
...
Процедура СертификатОплатыПриИзменении(Элемент)
	...
	СтруктураСертификата = ОбщегоНазначения.ПолучитьДанныеСертификата(СертификатОплатыПроверяемый);
...
&НаКлиенте
Процедура БлюдаПричинаОтказаПриИзменении(Элемент)
	
	СтрокаТЧ = Элементы.Блюда.ТекущиеДанные;
	Если ЗначениеЗаполнено(СтрокаТЧ.ПричинаОтказа) Тогда
		РасчетИтого();
	КонецЕсли;
...
Процедура ЗанятьСтол(Команда)
	...
	Если ЗначениеЗаполнено(Объект.НомерСтола) Тогда
		...
		СсылкаЗаказ = СоздатьДокументЗаказ(ПараметрыОбъекта);
		ИзменениеДоступностиКнопокФормы();
		Объект.Дата = ТекущаяДата();
		
		Элементы["Стол" + Объект.НомерСтола].Заголовок = Строка(Объект.НомерСтола) + Символы.ПС + Объект.Официант;
...
Процедура ОтправитьНаКухню(Команда)
	...
	ТабличнаяЧасть = Объект.Блюда;
	Для Каждого ТекущаяСтрока Из ТабличнаяЧасть Цикл
		Если (Не ЗначениеЗаполнено(ТекущаяСтрока.Блюдо) Или Не ЗначениеЗаполнено(ТекущаяСтрока.Количество)
			Или Не ЗначениеЗаполнено(ТекущаяСтрока.Цена)) 
			И Не ЗначениеЗаполнено(ТекущаяСтрока.ПричинаОтказа) Тогда
...
Процедура УстановитьРезерв(Команда)
	...
	Если ЗначениеЗаполнено(Объект.НомерСтола) Тогда
		...
		СсылкаЗаказ = СозданиеНовогоДокументаЗаказ(ТекущийОбъект);
...
Процедура ЗакрытьЗаказ(Команда)
	...
	Для каждого ТекущаяСтрока Из ТабличнаяЧасть Цикл
		 Если (Не ЗначениеЗаполнено(ТекущаяСтрока.Блюдо) Или
			Не ЗначениеЗаполнено(ТекущаяСтрока.Количество) ИЛИ
			Не ЗначениеЗаполнено(ТекущаяСтрока.Цена))
			И Не ЗначениеЗаполнено(ТекущаяСтрока.ПричинаОтказа) Тогда
...
&НаКлиенте
Процедура ОбновитьСостояниеСтолов(Команда)
...
&НаСервереБезКонтекста
Функция КоличествоЗаказовклиентаИРазмерСкидки(СсылкаКлиент)
	Возврат ОбщегоНазначения.КоличествоЗаказовклиентаИРазмерСкидки(СсылкаКлиент);
КонецФункции
...
Процедура ПрисвоениеЦветаНазванияСтолу()
	...
	|	И Заказы.НомерСтола = Столы.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтола,
	|	Резерв УБЫВ";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Резерв Тогда//Желтые
			Элементы["Стол" + ВыборкаДетальныеЗаписи.НомерСтола].Заголовок = Строка(ВыборкаДетальныеЗаписи.НомерСтола)
				+ Символы.ПС + "Резерв " + Символы.ПС + Формат(ВыборкаДетальныеЗаписи.ДатаРезерва, "ДФ='dd.MM.yy");
			Элементы["Стол" + ВыборкаДетальныеЗаписи.НомерСтола].ЦветФона = Новый Цвет(255, 255, 0);
		КонецЕсли;

		Если Не ВыборкаДетальныеЗаписи.ЗаказЗакрыт И Не ВыборкаДетальныеЗаписи.Резерв Тогда//Красные			
			Элементы["Стол" + ВыборкаДетальныеЗаписи.НомерСтола].Заголовок = Строка(ВыборкаДетальныеЗаписи.НомерСтола)
...
&НаСервере
Функция ЗаполнениеТЗЗаказа(НомерСтола)
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Если Не Объект.Официант = ВыборкаДетальныеЗаписи.Официант Тогда
		
			Сообщить("Этот заказ выполняет другой официант");
			Возврат Ложь;
...
&НаСервереБезконтекста
Функция СоздатьДокументЗаказ(ПараметрыОбъекта)
...
Процедура ИзменениеДоступностиКнопокФормы()
	...
	Если ЗначениеЗаполнено(СсылкаЗаказ) Тогда
		...
		Элементы.Блюда.Доступность = Истина;
		Элементы.ЗанятьСтол.Доступность = Ложь;
		Элементы.ОплатаСертификатом.Доступность = Истина;
		Элементы.Гость.Доступность = Истина;
	...
	Иначе
		...
		Элементы.ЗанятьСтол.Доступность = Истина;
		Элементы.ОплатаСертификатом.Доступность = Ложь;
		Элементы.Гость.Доступность = Ложь;
