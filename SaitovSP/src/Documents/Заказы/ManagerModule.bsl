...
Процедура РедактированиеДокумента (СсылкаЗаказ, Параметры, РежимЗаписи) Экспорт
	...
	Для каждого ТекущаяСТрокаТабличнойЧастиАРМа Из Параметры.Блюда Цикл
		...
		Если Не ТабличнаяЧастьСодержитИдентификатор(ДокРезерв.Блюда, ТекущаяСТрокаТабличнойЧастиАРМа.ИдентификаторСтроки) Тогда
		
			Если ТекущаяСТрокаТабличнойЧастиАРМа.Статус = СтатусБлюдаЗаказПринят 
				Или ЗначениеЗаполнено(ТекущаяСТрокаТабличнойЧастиАРМа.Статус) Тогда		
...
Функция ТабличнаяЧастьСодержитИдентификатор(ТЧПоиска, ИдентификаторСтроки)
	
	Для Каждого ТекущаяСтрока Из ТЧПоиска Цикл
		Если ТекущаяСТрока.ИдентификаторСтроки = ИдентификаторСтроки Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
